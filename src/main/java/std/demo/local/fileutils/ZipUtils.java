...
public class ZipUtils {
	...
	public static void main(String[] args) throws IOException, RarException {
		...
		// // zip(target, file);
		// zip("C:\\Users\\sinoadmin\\Desktop\\e.zip",
		// "C:\\Users\\sinoadmin\\Desktop\\mydate\\");

		unZip("C:\\Users\\sinoadmin\\Desktop\\5000000号码测试.rar", null);

	}

	public static void zip(String target, String directoryPath) throws IOException {
	...
	public static void zip(String parentPath, String target, ZipOutputStream out, File... files) throws IOException {
		for (File file : files) {
			if (file.isDirectory()) {
				// 是文件夹
				// 获取此文件夹下文件列表
				...
				if (listFiles.length > 0) {
					...
					if (parentPath != null) {
						emptyDirectoryPath = parentPath + File.separator + emptyDirectoryPath;
					}
			...
			ZipEntry entry = new ZipEntry(
					parentPath == null ? file.getName() : parentPath + File.separator + file.getName());
